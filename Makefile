...
clean: ## remove generated byte code, coverage reports, and build artifacts
	find ieia/ -name '__pycache__' -exec rm -rf {} +
	find ieia/ -name '*.pyc' -exec rm -f {} +
	find ieia/ -name '*.pyo' -exec rm -f {} +
	find ieia/ -name '*~' -exec rm -f {} +
	coverage erase
	rm -fr build/
	rm -fr dist/
	rm -fr *.egg-info
...
install-test: ## install requirements for tests
	...
	python setup.py develop --quiet  # XBlock plugin (ieia) has to be installed via entry_points.
...
STATIC_JS = ieia/xblock/static/js
STATIC_CSS = ieia/xblock/static/css
...
extract_translations: ## creates the django-partial.po & django-partial.mo files
	cd ./ieia && django-admin makemessages -l en -v1 -d django
	cd ./ieia && django-admin makemessages -l en -v1 -d djangojs
...
install_transifex_client: ## Install the Transifex client
	...
	# Load the PATH changes to make transifex client accessible from any directory i.e. ieia for pull translations
